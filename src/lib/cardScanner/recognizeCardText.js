...
export function normalizeOcrResult(result) {
  ...
  return { fullText: String(result.fullText ?? result.text ?? blocks.map((block) => block.text).join("\n")).trim(), blocks, ...(result.passes ? { passes: result.passes } : {}), ...(result.ocrMatch ? { ocrMatch: result.ocrMatch } : {}), ...(result.visualMatch ? { visualMatch: result.visualMatch } : {}), ...(result.visualError ? { visualError: result.visualError } : {}), ...(result.scannerTiming ? { scannerTiming: result.scannerTiming } : {}), ...(result.previewUrl ? { previewUrl: result.previewUrl } : {}), ...(result.originalPreviewUrl ? { originalPreviewUrl: result.originalPreviewUrl } : {}), ...(result.outlinePreviewUrl ? { outlinePreviewUrl: result.outlinePreviewUrl } : {}), ...(result.bottomPreviewUrl ? { bottomPreviewUrl: result.bottomPreviewUrl } : {}), ...(result.imageDiagnostics ? { imageDiagnostics: result.imageDiagnostics } : {}) };
}
