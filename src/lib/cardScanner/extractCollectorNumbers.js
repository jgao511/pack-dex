...
function correctParts(value) {
  const compact = value.replace(/\s+/g, "").toUpperCase();
  const prefix = compact.match(/^(TG|GG|SV|XY|SWSH)/)?.[0] || "";
  const choices = [...compact.slice(prefix.length)].map((character) => character === "B" ? ["3", "8"] : [character.replace(/O/g, "0").replace(/[IL]/g, "1").replace(/S/g, "5")]);
  return choices.reduce((values, next) => values.flatMap((value) => next.map((character) => value + character)), [prefix]).slice(0, 4);
}
function parseSegment(text, sourcePass = "full-card") {
  ...
  for (const match of normalizedText.matchAll(paired)) {
    for (const cardNumber of correctParts(match[1])) for (const printedSetTotal of correctParts(match[2])) {
      if (!/\d/.test(cardNumber) || !/\d/.test(printedSetTotal) || YEAR.test(cardNumber)) continue;
      const prefix = cardNumber.match(/^[A-Z]+/)?.[0] || "";
      results.push({ raw: match[0], cardNumber, printedSetTotal, prefix, numericComponent: Number(cardNumber.match(/\d+/)?.[0]), normalized: normalizeCollectorNumber(cardNumber), normalizedTotal: normalizeCollectorNumber(printedSetTotal), sourcePass });
    }
