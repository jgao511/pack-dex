...
export function fuseCardMatches(ocrMatch, visualMatch, catalog = getScannerCatalog()) {
  ...
  const ocrCompatibleIds = new Set([...(ocrMatch?.narrowedCardIds || []), ...ocrById.keys()]);
  ...
  const ids = [...new Set([...lightById.keys(), ...ocrById.keys(), ...orbById.keys()])];
  const scored = ids.map((cardId) => {
    ...
    const ocrCompatible = ocrCompatibleIds.has(cardId);
    const ocrScore = clamp((ocr?.score || 0) / 110); const lightScore = light?.score || 0; const orbScore = orb?.score || 0;
    let fusedScore = ocr ? ocrScore * .44 + lightScore * .26 + orbScore * .3 : lightScore * .38 + orbScore * .52 + (ocrCompatible ? .1 : 0);
    ...
    if (!ocr && ocrCompatible) reasons.push("OCR-compatible name or collector number");
    ...
    return { entry, ocr, ocrCompatible, light, orb, score: clamp(fusedScore), reasons };
  }).filter(Boolean).filter((item) => item.ocrCompatible || (item.light?.score >= .72 && item.orb?.score >= .35)).sort((a, b) => b.score - a.score || a.entry.cardId.localeCompare(b.entry.cardId));
