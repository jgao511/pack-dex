...
test("catalog card paths resolve through the public set asset CDN path", () => {
  ...
  assert.equal(resolved, "https://assets.pack-dex.com/sets/mega-evolution/cards/58_Ralts_Common.png");
  assert.equal(new URL(resolved).hostname, "assets.pack-dex.com");
  assert.notEqual(new URL(resolved).hostname, "pack-dex.com");
});

test("set-relative paths use the public set asset CDN path and absolute URLs remain unchanged", () => {
  assert.equal(
    getSetAssetUrl("mega-evolution/cards/58_Ralts_Common.png"),
    "https://assets.pack-dex.com/sets/mega-evolution/cards/58_Ralts_Common.png",
  );
  assert.equal(
