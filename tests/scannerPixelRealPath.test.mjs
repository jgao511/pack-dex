...
test("native proposal processing starts from the full capture and keeps OCR per proposal", async () => {
  const adapter = await readFile(new URL("../mobile-app/src/lib/nativeScannerAdapters.js", import.meta.url), "utf8");
  assert.match(adapter, /analyzeProposalCanvases\(fullCanvas/);
  assert.match(adapter, /mappedCrop\.x \* fullCanvas\.width \/ originalWidth/);
  assert.match(adapter, /for \(const proposal of proposals\)[\s\S]*recognizeProposalPasses\(proposal\.canvas, \["full-card"\]\)/);
