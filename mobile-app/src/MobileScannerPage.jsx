...
import { decideScannerAcceptance, SCANNER_ACCEPTANCE_MODES } from "../../src/lib/cardScanner/scannerAcceptancePolicy.js";
...
function scannerEvidenceDebug(recognized, fused) {
  if (!import.meta.env.DEV) return;
  const candidates = recognized?.frozenA?.candidates || []; const top1 = candidates[0]?.score; const top2 = candidates[1]?.score;
  scannerDebug("frozen-a-evidence", {
    top1: Number.isFinite(top1) ? top1 : null, top2: Number.isFinite(top2) ? top2 : null,
    top1Top2Margin: Number.isFinite(top1) && Number.isFinite(top2) ? top1 - top2 : null,
    top3: candidates.slice(0, 3).map(({ cardId, score }) => ({ cardId, score })),
    ocrTextAvailable: Boolean(recognized?.text?.trim()), ocrNameEvidence: recognized?.ocrMatch?.nameCandidates?.length || 0,
    collectorNumberEvidence: recognized?.ocrMatch?.collectorNumbers?.length || 0,
    geometry: recognized?.imageDiagnostics || null,
    finalTopThreeCardIds: fused?.results?.slice(0, 3).map(({ cardId }) => cardId) || [],
  });
}
...
export default function MobileScannerPage({ authState, authUserId, onRequireAuth, onAddToCollection, onAddToWishlist, onLoadActionState, onSearchManually, onLoadCardPrice }) {
  ...
  async function analyze(image, browser = false) {
    ...
    try {
      ...
      const decision = decideScannerAcceptance(recognized, fused);
      const policyMs = performance.now() - policyStarted;
      scannerEvidenceDebug(recognized, decision.match);
      scannerDebug("analysis-complete", { adapter: browser ? "browser-production-visual" : "nativeOcrAdapter", totalMs: performance.now() - started, policyMs, acceptanceMode: decision.mode, rawOcrCandidates: recognized.ocrMatch?.results?.length || 0, rawVisualCandidates: recognized.visualMatch?.lightweight?.candidates?.length || 0, normalizedCandidates: decision.match.results.length, scannerTiming: recognized.scannerTiming || null });
      if (analysis !== analysisRef.current) return;
      if (decision.mode === SCANNER_ACCEPTANCE_MODES.NO_MATCH) { setStage("no-match"); return; }
      setMatch(decision.match); setSelectedCardId(decision.match.results[0].cardId); setStage("candidates");
    } catch (scanError) {
      if (analysis !== analysisRef.current) return;
