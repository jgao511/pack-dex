...
function AiCandidateWithImage({ result, onSelect }) {
  const [imageFailed, setImageFailed] = useState(false);
  const imageUrl = getCardImageUrl({ imageUrl: result.imageUrl });
  const collector = result.collectorNumber ? `#${result.collectorNumber}${result.printedSetTotal ? `/${result.printedSetTotal}` : ""}` : "Collector number unavailable";
  return <article className="scanner-candidate">{imageUrl && !imageFailed ? <img src={imageUrl} alt={`${result.name} candidate`} onError={() => setImageFailed(true)} /> : <div className="scanner-candidate-placeholder" role="img" aria-label="Card image unavailable">Card image unavailable</div>}<div><h3>{result.name}</h3><p>{result.setName || "Set unavailable"} · {collector}</p><p>Rank #{result.displayRank || "?"} · {result.confidence || "User confirmation required"}</p><p>Similarity {Number(result.visualScore ?? result.score ?? 0).toFixed(3)}{Number.isFinite(result.margin) && ` · margin ${result.margin.toFixed(3)}`}</p><button className="secondary-action" type="button" onClick={() => onSelect(result)}>Select Card</button></div></article>;
}
...
export default function CardScannerDevPage({ nativeCameraAdapter, ocrAdapter } = {}) {
  ...
  async function fileFromCapturedImage(captured) {
    if (captured?.file instanceof File) return captured.file;
    const response = await fetch(captured?.imageUrl);
    if (!response.ok) throw new Error("We couldn’t open the captured scanner image.");
    const blob = await response.blob();
    return new File([blob], `scanner-capture-${Date.now()}.jpg`, { type: blob.type || "image/jpeg" });
  }
  async function beginCapture(source) {
    ...
    try {
      ...
      if (aiPocEnabled) {
        const file = await fileFromCapturedImage(nextImage); nextImage.release?.();
        await runAiFile(file); return;
      }
  ...
  return <main className={`scanner-dev ${stage === "start" ? "preview-active" : ""}`}>
    ...
    {stage === "start" && <div className="scanner-primary-actions"><button className="scanner-shutter" type="button" aria-label="Capture card" disabled={activeCameraAdapter.isAvailable() && !previewStart?.previewStarted} onClick={capturePreview}><span /></button><button className="secondary-action" type="button" onClick={() => beginCapture("library")}>Choose Photo</button><button className="secondary-action" type="button" onClick={runReferenceTest}>Run Reference Test</button>{pixelFixtureUrls.map((_, index) => <button className="secondary-action" type="button" key={index} onClick={() => runPixelFixtureTest(index)}>Run Pixel Fixture {index + 1}</button>)}{aiPocEnabled && <><button className="secondary-action" type="button" onClick={chooseAiPhoto}>Try Scanner-AI Photo</button><button className="secondary-action" type="button" onClick={runAiPocStatus}>Run Scanner-AI Reference</button><small>Scanner-AI quality guidance is evaluated after capture, not during live preview.</small></>}<label className="scanner-preview-diagnostic"><input type="checkbox" checked={previewInFront} onChange={(event) => setPreviewInFront(event.target.checked)} /> Diagnostic: preview in front</label>{previewStart?.previewStarted && <small>Embedded preview started · {Math.round(previewStart.previewWidth)} × {Math.round(previewStart.previewHeight)}</small>}</div>}
