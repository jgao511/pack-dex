...
function MobileApp({
  ...
  async function performAccountScopedStateLoad(currentUser) {
    ...
    try {
      persistedBinders = await loadPersistedBinders(currentUser.id);
    } catch (error) {
      console.warn("Unable to load mobile cloud binders", {
        userId: currentUser.id,
        error,
      });
    }
  ...
  function persistMobileBinderState(nextBinders, changedBinderId = "") {
    persistBindersForUser({
      ...
      console.warn("Unable to save mobile binder", {
        userId: user?.id || "",
        binderId: changedBinderId,
        error,
      });
    });
  }
  ...
  function createCustomBinder(name, theme = "midnight") {
    ...
    persistMobileBinderState(nextBinders, nextBinder.id);
  }
  ...
  function addCardsToCustomBinder(binderId, selections) {
    setBinders((currentBinders) => {
      ...
      if (nextBinders !== currentBinders) persistMobileBinderState(nextBinders, binderId);
      return nextBinders;
    });
  }
  ...
  function replaceCustomBinderCards(binderId, cards) {
    setBinders((currentBinders) => {
      ...
      persistMobileBinderState(nextBinders, binderId);
      return nextBinders;
    });
  }
