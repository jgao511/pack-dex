...
begin
  ...
  loop
    ...
    insert into public.user_collection_increment_events as event_row(user_id, client_event_id)
    ...
    select coalesce(sum(new_card.grouped_quantity), 0)::integer,
           count(*) filter (where new_card.is_new)::integer
      into v_total_quantity, v_new_card_count
    from new_cards as new_card;
